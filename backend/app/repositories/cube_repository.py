...
class CubeRepository:
    ...
    async def get_strand_rollup_for_item(
        ...
        Sources from ``cube_standard_summary`` (the pre-rolled-up cube with
        one physical row per (school_id, item_id, identifier)) joined to
        ``dim_standard`` for the strand label. Each identifier contributes
        its ``total_questions`` (per-identifier distinct count, already
        deduped at cube build time) and its ``grade_average``; the strand-
        level number-correct is the question-weighted average of those
        identifier averages.

        Empty/NULL strand rows are filtered out before aggregation so the
        rendered table never carries a sentinel blank-name row. NULL-
        identifier cube rows (typology buckets generated by the pipeline)
        are excluded — they have no identifier to map back to a strand.
        """
        # cube_standard_summary has total_questions per (item_id, identifier)
        # already deduplicated; dim_standard.strand is the canonical
        # strand label tied to the identifier.
        sql = text(
            """
            WITH item_std AS (
                SELECT
                    ...
                    css.total_questions,
                    css.grade_average
                FROM cube_standard_summary css
                JOIN dim_standard ds
                  ON ds.identifier = css.identifier
                WHERE css.item_id = :item_id
                  AND css.identifier IS NOT NULL
                  AND ds.strand IS NOT NULL
                  AND ds.strand <> ''
            ),
            -- Collapse multi-row aliases per (strand, identifier) so each
            -- identifier is counted exactly once toward the strand totals.
            per_identifier AS (
                SELECT
                    strand,
                    identifier,
                    ...
                    AVG(grade_average)   AS grade_average
                FROM item_std
                GROUP BY strand, identifier
            )
            SELECT
                strand,
                COUNT(DISTINCT identifier)                                    AS num_standards,
                SUM(total_questions)                                          AS num_questions,
                CASE WHEN SUM(total_questions) > 0
                     THEN SUM(total_questions * grade_average)
    ...
    async def get_standard_rollup_for_item(
        ...
        identifier (e.g. ``AR.3.1`` and ``912.AR.3.1`` both stem from
        identifier ``3cd52b67…``). Sources directly from
        ``cube_standard_summary`` joined to ``dim_standard`` for the
        cpalms / schoology labels; uses
        ``cube_standard_summary.grade_average`` straight (it already
        carries the per-(item, identifier) average computed by the cube
        builder).

        NULL-identifier cube rows are excluded — they cannot map to a
        cpalms label and would otherwise produce a blank-key row.
        """
        sql = text(
            """
            SELECT
                ds.cpalms_standard                                            AS cpalms_standard,
                COALESCE(ds.schoology_standard, '')                           AS schoology_standard,
                ds.strand                                                     AS strand,
                MAX(css.total_questions)                                      AS num_questions,
                AVG(css.grade_average)                                        AS grade_average
            FROM cube_standard_summary css
            JOIN dim_standard ds
              ON ds.identifier = css.identifier
            WHERE css.item_id = :item_id
              ...
              AND ds.strand IS NOT NULL
              AND ds.strand <> ''
              AND COALESCE(NULLIF(ds.cpalms_standard, ''),
                           NULLIF(ds.schoology_standard, ''), '') <> ''
            GROUP BY ds.strand, ds.cpalms_standard, ds.schoology_standard
            ORDER BY ds.strand, ds.cpalms_standard
            """
        )
        result = await self.session.execute(sql, {"item_id": item_id})
        return [_row_to_dict(r) for r in result.all()]
    ...
    async def get_standard_bands_for_item(
        ...
        sql = text(
            ...
            WHERE css.item_id = :item_id
              ...
              AND ds.strand <> ''
              AND COALESCE(NULLIF(ds.cpalms_standard, ''), '') <> ''
            GROUP BY ds.cpalms_standard, ds.strand
            ORDER BY ds.cpalms_standard
            """
        )
        result = await self.session.execute(sql, {"item_id": item_id})
        return [_row_to_dict(r) for r in result.all()]
