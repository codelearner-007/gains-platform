...
async def test_runner_guard_blocks_unscoped_truncate_when_locked(monkeypatch) -> None:
    """With ``any_locked()`` → True, scanning the REAL TRANSFORMATIONS_ORDER must
    raise, naming an offending unscoped-TRUNCATE file. No transforms run."""
    repo = _FakeLockRepo(any_locked=True)
    _patch_runner_repo(monkeypatch, repo)
    base = runner_mod._base_dir()

    with pytest.raises(RuntimeError) as exc:
        await runner_mod._assert_no_locked_truncate(session=object(), base=base, only_tag=None)
...
@pytest.mark.parametrize("only_tag", ["cubes", "facts"])
async def test_runner_guard_blocks_tagged_rebuild_when_locked(monkeypatch, only_tag) -> None:
    """The three named wipe-attempts (MASTER_PLAN §TEST): a locked env must refuse
    not just a full rebuild but ALSO a scoped ``--tag cubes`` / ``--tag facts``
    rebuild — each of those tags contains an unscoped TRUNCATE of a session-bearing
    table (cube_* / fact_student_submission). Proven against the REAL order."""
    repo = _FakeLockRepo(any_locked=True)
    _patch_runner_repo(monkeypatch, repo)
    base = runner_mod._base_dir()
    ...
    with pytest.raises(RuntimeError) as exc:
        await runner_mod._assert_no_locked_truncate(
            session=object(), base=base, only_tag=only_tag
        )

    msg = str(exc.value)
    assert "historic-lock" in msg
    assert "TRUNCATE" in msg
    assert ".sql" in msg  # names a concrete offending file within the tag
    assert repo.any_locked_calls == 1
...
async def test_runner_guard_inert_when_no_locks(monkeypatch) -> None:
    ...
    result = await runner_mod._assert_no_locked_truncate(
        session=object(), base=base, only_tag=None
    )
...
def test_runner_truncated_session_tables_flags_real_offender() -> None:
    """Unit sanity: the real fact SQL DOES contain an unscoped TRUNCATE of a
    session-bearing table (so the guard has something to catch)."""
    base = runner_mod._base_dir()
    fact_sql = (base / "07_facts/fact_student_submission.sql").read_text(encoding="utf-8")
    hits = runner_mod._truncated_session_tables(fact_sql)
    assert "fact_student_submission" in hits
