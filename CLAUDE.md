...
- `docs/audit/fixes/03_prod_data_sync_runbook.md` — **canonical procedure for changing prod DATA** (supersedes 02's convergence approach): Method A full reload (make prod == verified local via TRUNCATE + chunked COPY) and Method B surgical delete + cube-reload, with the environment gotchas that bite (pooler drops long statements, FK-skip via `session_replication_role=replica`, drop-index/load/recreate for fact, `VACUUM` after COPY, migration CI on main-push, pg17-Docker rollback). Read before ANY prod data change or backend deploy.
